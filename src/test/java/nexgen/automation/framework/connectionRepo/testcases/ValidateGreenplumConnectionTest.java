...
import nexgen.automation.framework.connectionRepo.GreenplumConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidateGreenplumConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/Greenplum.xlsx";
	...
	@Test(groups ="Smoke")
	public void VerifycreateGreenplumConnection() throws Exception
	{
		test = extent.createTest("VerifycreateGreenplumConnection");
		...
		GreenplumConnection green = new GreenplumConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			green.createGreenplumConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
			output.get(8),output.get(0),soft,output.get(9),output.get(11),output.get(12),output.get(13),connection,rowNumber, 
			testCaseStatus,"Status","Create-Connection","VerifycreateGreenplumConnection",test,log,output.get(14));
	...
	public void VerifyupdateGreenplumConnection() throws Exception {
		test = extent.createTest("VerifycreateGreenplumConnection");
		SoftAssert soft = new SoftAssert();
	...
	//	if (loginTest.login(username, password))
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			green.updateGreenplumConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
					...
					testCaseStatus, "Status",log,test, "VerifyupdateGreenplumConnection");
	...
	public void VerifyGreenplumConnectionMandatory() throws Exception {
		
		test = extent.createTest("Greenplum_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			green.GreenplumMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
					...
					log,"GreenplumMandatoryField", testCaseStatus);
		}

		soft.assertAll();
		green.testCaseStatus(testCaseStatus, log);
