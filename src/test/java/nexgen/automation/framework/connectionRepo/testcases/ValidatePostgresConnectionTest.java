...
import nexgen.automation.framework.connectionRepo.PostgresConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidatePostgresConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/Postgres.xlsx";
	...
	public void VerifycreateMysqlConnection() throws Exception
	{
		test = extent.createTest("VerifycreateMysqlConnection");
		...
		PostgresConnection mysql = new PostgresConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.createPostgresConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
			output.get(8),output.get(0),soft,output.get(9),output.get(11),output.get(12),output.get(13),connection,rowNumber, 
			testCaseStatus,"Status","Create-Connection","VerifycreateMysqlConnection",test,log,output.get(14));
	...
	@Test(groups = { "DepthFirst", "ConnectionRepo", "testcases" })
	public void VerifyupdateMysqlConnection() throws Exception {
		test = extent.createTest("VerifycreateMysqlConnection");
		SoftAssert soft = new SoftAssert();
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			mysql.updatePostgresConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
					...
					testCaseStatus, "Status",log,test, "VerifyupdateMysqlConnection");
	...
	public void VerifymySqlConnectionMandatory() throws Exception {
		
		test = extent.createTest("Postgres_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.PostgresMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
					...
					log,"mySqlMandatoryField", testCaseStatus);
		}
