...
import nexgen.automation.framework.connectionRepo.HPVerticaConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidateHPVerticaConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/HPVertica.xlsx";
	...
	@Test(groups ="Smoke")
	public void VerifycreateHPVerticaConnection() throws Exception
	{
		test = extent.createTest("VerifycreateHPVerticaConnection");
		...
		HPVerticaConnection mysql = new HPVerticaConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.createHPVerticaConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
			output.get(8),output.get(0),soft,output.get(9),output.get(11),output.get(12),output.get(13),connection,rowNumber, 
			testCaseStatus,"Status","Create-Connection","VerifycreateHPVerticaConnection",test,log,output.get(14));
	...
	public void VerifyupdateHPVerticaConnection() throws Exception {
		test = extent.createTest("VerifycreateHPVerticaConnection");
		SoftAssert soft = new SoftAssert();
		...
		HPVerticaConnection HPVertica = new HPVerticaConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			HPVertica.updateHPVerticaConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
					...
					testCaseStatus, "Status",log,test, "VerifyupdateHPVerticaConnection");
	...
	public void VerifyHPVerticaConnectionMandatory() throws Exception {
		
		test = extent.createTest("HPVertica_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			HPVertica.HPVerticaMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
					...
					log,"HPVerticaMandatoryField", testCaseStatus);
		}

		soft.assertAll();
		HPVertica.testCaseStatus(testCaseStatus, log);
