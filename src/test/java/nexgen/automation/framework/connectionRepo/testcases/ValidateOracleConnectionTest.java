...
import nexgen.automation.framework.connectionRepo.OracleConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidateOracleConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/Oracle.xlsx";
	...
	public void VerifycreateMysqlConnection() throws Exception
	{
		...
		OracleConnection mysql = new OracleConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.createOracleConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
	...
	public void VerifyupdateMysqlConnection() throws Exception {
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			mysql.updateOracleConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
	...
	public void VerifymySqlConnectionMandatory() throws Exception {
		...
		test = extent.createTest("Oracle_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.OracleMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
