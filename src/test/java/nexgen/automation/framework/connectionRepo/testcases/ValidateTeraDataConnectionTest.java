...
import nexgen.automation.framework.connectionRepo.TeraDataConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidateTeraDataConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/Teradata.xlsx";
	...
	@Test(groups ="Smoke")
	public void VerifycreateTeradataConnection() throws Exception
	{
		test = extent.createTest("VerifycreateTeradataConnection");
		...
		TeraDataConnection teradata = new TeraDataConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			teradata.createTeradataConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
			output.get(8),output.get(0),soft,output.get(9),output.get(11),output.get(12),output.get(13),connection,rowNumber, 
			testCaseStatus,"Status","Create-Connection","VerifycreateTeradataConnection",test,log,output.get(14));
	...
	public void VerifyupdateTeradataConnection() throws Exception {
		test = extent.createTest("VerifycreateTeradataConnection");
		SoftAssert soft = new SoftAssert();
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			teradata.updateTeradataConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
					...
					testCaseStatus, "Status",log,test, "VerifyupdateTeradataConnection");
	...
	public void VerifyTeradataConnectionMandatory() throws Exception {
		
		test = extent.createTest("Teradata_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			teradata.TeradataMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
					...
					log,"TeradataMandatoryField", testCaseStatus);
		}

		soft.assertAll();
		teradata.testCaseStatus(testCaseStatus, log);
