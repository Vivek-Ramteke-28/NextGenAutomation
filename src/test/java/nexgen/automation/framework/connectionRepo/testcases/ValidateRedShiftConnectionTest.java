...
import nexgen.automation.framework.connectionRepo.RedShiftConnection;
//import nexgen.automation.pageclasses.pages.LoginPage;

@Listeners(ListenerTest.class)
public class ValidateRedShiftConnectionTest extends BaseSuite{
	
	String connection = System.getProperty("user.dir") + "/src/main/resources/testdata/Connections/Redshift.xlsx";
	...
	@Test(groups ="Smoke")
	public void VerifycreateRedshiftConnection() throws Exception
	{
		test = extent.createTest("VerifycreateRedshiftConnection");
		...
		RedShiftConnection mysql = new RedShiftConnection(driver);
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.createRedshiftConnection(driver,output.get(1),output.get(10),output.get(2),output.get(3),output.get(4),output.get(5),output.get(6),output.get(7),
			output.get(8),output.get(0),soft,output.get(9),output.get(11),output.get(12),output.get(13),connection,rowNumber, 
			testCaseStatus,"Status","Create-Connection","VerifycreateRedshiftConnection",test,log,output.get(14));
	...
	public void VerifyupdateRedshiftConnection() throws Exception {
		test = extent.createTest("VerifycreateRedshiftConnection");
		SoftAssert soft = new SoftAssert();
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			List<String> output = XlReader.getDetails(inputs, rowNumber, "Create-Connection", XlReader, connection);
			mysql.updateRedshiftConnection(driver, output.get(1),output.get(15),output.get(6), output.get(2), output.get(3),
					...
					testCaseStatus, "Status",log,test, "VerifyupdateRedshiftConnection");
	...
	public void VerifyRedshiftConnectionMandatory() throws Exception {
		
		test = extent.createTest("Redshift_Mandatory");
		...
		for (int rowNum = 0; rowNum < rowCount; rowNum++) {
			...
			mysql.RedshiftMandatoryField(soft, output.get(0), output.get(1), output.get(2), output.get(3), output.get(4),
					...
					log,"RedshiftMandatoryField", testCaseStatus);
		}
